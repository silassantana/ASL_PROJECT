...
def _compute_global_stats(h5_path, split, max_seq_len=None, sample_cap=2000):
    ...
    with h5py.File(h5_path, "r") as f:
        ...
        n = len(ds)
        feature_dim = (
            ds.shape[2] if ds.ndim == 3 else int(f.attrs.get("feature_dim", 512))
        )
...
class H5Dataset(Dataset):
    ...
    def __init__(
        ...
        with h5py.File(h5_path, "r") as f:
            ...
            if len(sample_seq.shape) == 2:
                ...
                self.feature_dim = int(f.attrs.get("feature_dim", 512))

            # Get gloss mapping (check multiple locations)
            if "gloss_names" in f:
            ...
            if self.max_seq_len is not None:
                self._effective_seq_lengths = np.minimum(
                    self._seq_lengths, self.max_seq_len
                )
            else:
            ...
            # Clamp potentially corrupted metadata into safe bounds.
            self._seq_lengths = np.clip(self._seq_lengths, 0, max_seq_width)
            self._effective_seq_lengths = np.clip(
                self._effective_seq_lengths, 0, max_seq_width
            )
            self._label_lengths = f[f"{split}_label_lengths"][:].astype(np.int32)

            # Recover corrupted label lengths when possible.
            # Heuristic: if stored length is out of bounds, use the last non-zero token + 1.
            recovered = 0
            invalid_len = (self._label_lengths < 0) | (
                self._label_lengths > max_label_width
            )
            ...
            if recovered > 0:
                print(
                    f"[{split}] Recovered label lengths for {recovered} samples.",
                    flush=True,
                )
    ...
    def __getitem__(self, idx):
        ...
        if self._is_flat:
            ...
            sequence = self.seqs_ds[real_idx, :seq_len, :].astype(np.float32)
...
def train_epoch(
    ...
    for step, (
        ...
        with torch.amp.autocast(device_type="cuda", enabled=amp_enabled):
            logits = model(
                sequences, decoder_input, src_key_padding_mask=src_padding_mask
            )  # [batch, max_len+1, vocab_size]
        ...
        if not torch.isfinite(loss):
            ...
            if amp_enabled:
                amp_enabled = False
                pbar.write(
                    "Encountered non-finite AMP loss; falling back to full precision."
                )
        ...
        if should_step:
            if scaler is not None and amp_enabled:
                ...
                if not torch.isfinite(grad_norm):
                    ...
                    amp_enabled = False
                    pbar.write(
                        "Encountered non-finite AMP gradients; falling back to full precision."
                    )
                    continue
...
def evaluate(
    ...
    with torch.no_grad():
        for sequences, labels, seq_lengths, label_lengths, src_padding_mask in tqdm(
            loader, desc=desc
        ):
            ...
            predictions = model.generate(
                memory, max_length=30, memory_key_padding_mask=src_padding_mask
            )
    ...
    for _ in range(max_length):
        ...
        for toks, score, finished in beams:
            ...
            topk_logp, topk_idx = torch.topk(
                log_probs, k=min(beam_size, log_probs.numel())
            )
...
def _seed_worker(worker_id, base_seed=42):
    """Worker init fn must be top-level to be picklable on Windows spawn."""
...
def _resolve_device(device_arg):
    """Resolve runtime device with optional DirectML support.

    Returns:
        (device, backend_name)
    """
    dev = str(device_arg).lower().strip()
    if dev == "cuda":
    ...
    if dev == "dml":
        if torch_directml is None:
            raise RuntimeError(
                "DirectML requested but torch_directml is not installed."
            )
...
def main():
    ...
    parser.add_argument(
        "--num-encoder-layers", type=int, default=3, help="Number of encoder layers"
    )
    parser.add_argument(
        "--num-decoder-layers", type=int, default=3, help="Number of decoder layers"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Enable channel attention for keypoint features",
    )
    parser.add_argument(
        "--attention-reduction",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Validation batch size (default: max(1, batch_size//2))",
    )
    parser.add_argument("--epochs", type=int, default=100, help="Number of epochs")
    parser.add_argument("--lr", type=float, default=1e-4, help="Learning rate")
    parser.add_argument("--warmup-epochs", type=int, default=5, help="Warmup epochs")
    parser.add_argument(
        "--grad-accum-steps", type=int, default=4, help="Gradient accumulation steps"
    )
    parser.add_argument(
        "--max-seq-len",
        type=int,
        default=512,
        help="Cap on input frame length per sample",
    )
    parser.add_argument(
        "--num-workers",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=128,
        help="Per-worker HDF5 raw chunk cache in MB",
    )
    parser.add_argument(
        "--amp", action="store_true", help="Enable automatic mixed precision on CUDA"
    )
    parser.add_argument(
        "--label-smoothing",
        type=float,
        default=0.05,
        help="Label smoothing for cross-entropy loss (default: 0.05)",
    )
    parser.add_argument(
        "--lr-restart-every",
        type=int,
    ...
    parser.add_argument(
        "--class-weight-min", type=float, default=0.2, help="Minimum class weight clamp"
    )
    parser.add_argument(
        "--class-weight-max", type=float, default=5.0, help="Maximum class weight clamp"
    )
    parser.add_argument(
        "--early-stop-patience",
        type=int,
    ...
    parser.add_argument(
        "--save-dir", type=str, default="checkpoints_encdec", help="Save directory"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Device backend: auto, cuda, dml (DirectML), or cpu",
    )
    parser.add_argument(
        "--low-vram-2gb",
        action="store_true",
        help="Apply conservative settings for ~2GB GPUs (smaller batches/workers/seq len)",
    )
    ...
    if args.low_vram_2gb:
        # Conservative defaults for tiny VRAM; user can still override via CLI.
        if args.batch_size > 2:
            args.batch_size = 2
        if args.eval_batch_size is None:
            args.eval_batch_size = 1
        if args.grad_accum_steps < 8:
            args.grad_accum_steps = 8
        if args.max_seq_len is None or args.max_seq_len > 192:
            args.max_seq_len = 192
        if args.num_workers > 0:
            args.num_workers = 0
        args.h5_cache_mb = min(args.h5_cache_mb, 64)
    ...
    elif backend == "dml" and args.amp:
        print("AMP is CUDA-only in this script; disabling AMP on DirectML.")
        args.amp = False
    ...
    train_mean, train_std = _compute_global_stats(
        args.data, "train", max_seq_len=args.max_seq_len
    )
    ...
    eval_batch_size = (
        ...
        else max(1, args.batch_size // 2)
    )
    collate = partial(collate_fn, max_seq_len=None)

    _nw = args.num_workers
    _pin = device.type == "cuda"
    worker_init_fn = partial(_seed_worker, base_seed=args.seed) if _nw > 0 else None

    train_batch_sampler = BucketBatchSampler(
        train_dataset._effective_seq_lengths[train_dataset._valid_indices],
    ...
    if args.use_class_weights:
        ...
        print(
            f"Using class weights: min={wmin:.3f}, max={wmax:.3f}, power={args.class_weight_power}"
        )
    ...
    if args.resume:
        ...
        if os.path.exists(resume_path):
            ...
            best_bleu = resume_ckpt.get(
                "best_bleu", resume_ckpt.get("best_gloss_recall", 0.0)
            )
    ...
    print(
        f"Train batch: {args.batch_size}, Eval batch: {eval_batch_size}, Grad accum: {args.grad_accum_steps}, AMP: {args.amp and device.type == 'cuda'}"
    )
    if args.eval_beam_size > 1:
        print(
            f"Validation decoding: beam={args.eval_beam_size}, length_penalty={args.eval_length_penalty}"
        )
    else:
